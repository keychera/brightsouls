...
/* Jika SbX bernilai false, maka dicerminkan terhadap sumbu Y */
{
    if (SbX == true) {
        ...
        return MakePOINT(0-Absis(P),Ordinat(P));
    }
}
...
/* F.S. P digeser sebesar Sudut derajat dengan sumbu titik (0,0) */
{
    ...
    float teta = asin(Ordinat(*P)/Jarak0(*P));
    if (teta>0 && Absis(*P)<0) {
        teta = pi - teta;
    }
    else if (teta<0 && Absis(*P)<0) {
        teta = -(pi + teta);
    }
    teta += Sudut/180*pi;
    if (IsOrigin(*P) == false) {
        *P = MakePOINT((Jarak0(*P)*cos(teta)),(Jarak0(*P)*sin(teta)));
    }
}
